...
<body>
    ...
    <h4>Tổng tiền: <strong id="cartTotal">
        <?= !empty($cart_items) ? number_format(array_sum(array_column($cart_items, 'total')), 0, ',', '.') . ' đ' : '0 đ'; ?>
    </strong></h4>
</div>

<?php if (!empty($cart_items)) : ?>
    <a href="http://localhost/duAn1_HDPlus/index.php?act=addbill" class="btn btn-success w-100 mt-3">Thanh toán</a>
<?php else : ?>
